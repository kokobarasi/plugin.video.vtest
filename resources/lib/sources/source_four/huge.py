...
search_path_huge = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_huge.json")
mode_key  = os.path.join(xbmc.translatePath('special://home/'), keys_path + "modelist.json")
search_file_huge = os.path.join(xbmc.translatePath('special://home/'), search_path_huge + "mysearch.txt")
...
path_dicts, mode_huge_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'hugehome')]
path_dict = load_keys()
...
def readWrite_huge(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_huge,'w') as file_handle:
        ...
        with open(search_file_huge, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if m_part.endswith('.com/'):
        ...
        if not '/search/' in m_part and num <= 1:
            ...
            readWrite_huge(arg_str = '-'.join(name.lower().split()), write=True)
                
            initial_url = m_part, params

        elif m_part.endswith('/search/') and num > 1:

            name = m_part + readWrite_huge() + '?from=' + str(num)
