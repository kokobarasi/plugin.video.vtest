...
search_path_phat = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_phat.json")
mode_key  = os.path.join(xbmc.translatePath('special://home/'), keys_path + "modelist.json")
search_file_phat = os.path.join(xbmc.translatePath('special://home/'), search_path_phat + "mysearch.txt")
...
path_dicts, mode_phat_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'phathome')]
path_dict = load_keys()
...
def readWrite_phat(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_phat,'w') as file_handle:
        ...
        with open(search_file_phat, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if m_part.endswith('popular/') and num > 1 and not 'search' in m_part:
        
        initial_url = re.sub('popular','trending',m_part) + str(num) + '/'
        ...
        readWrite_phat(arg_str = '-'.join(name.lower().split()), write=True)

        initial_url = m_part, params

    elif not m_part.endswith('popular/') and 'search' in m_part and num > 1:

        name = m_part + readWrite_phat() + '/' + str(num) + '/'
...
def name_site(): 
    """""" 
    page_download = WebCrawler('https://www.pornhat.com/models/')

    name_dict = page_download.fetch_names(selector='.thumb-ctr a', 
                                      title='title', 
                                      arg = 3, arg_num = 286,
                                      query_string='/'
                                      )
...
def channel_site(): 
    """""" 
    page_download = WebCrawler('https://www.pornhat.com/channels/')

    cat_dict = page_download.fetch_list(selector='.thumb-ctr a',
                                   attr='href',
                                   title='title',
                                   img='src',
                                   attr_option = '/sites/',
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    mode = re.compile(r"mode=(\w+)?&").search(args).group(1)
    if   mode   == "phatnames":      site_name(name_site)
    elif mode   == "phatchannel":    channel_name(channel_site)
