...
search_path_xbabe = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'
search_file_xbabe = os.path.join(xbmc.translatePath('special://home/'), search_path_xbabe + "mysearch.txt")

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_xbabe.json")
...
path_dicts, mode_xbabe_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'xbabehome')]
path_dict = load_keys()
...
def readWrite_xbabe(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_xbabe,'w') as file_handle:
        ...
        with open(search_file_xbabe, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if num > 1 and not '/search/?q=' in m_part:
        
        initial_url = m_part + str(num) + '/'

    elif num <= 1 and not '/search/?q=' in m_part:

        initial_url = m_part

    elif  m_part.endswith('/search/?q=') and num <= 1:

        name = xbabe_entry()

        params['q'] = name

        readWrite_xbabe(arg_str = '+'.join(name.lower().split()), write=True)

        initial_url = m_part, params

    elif '/search/?q=' in m_part and num > 1:

        name = m_part + str(num) + '/?q=' + readWrite_xbabe()
...
def fetch_links(m_part, min_search = 1, max_search = 2):
    ...
    movie_src = page_download.playable_links_tup(
                                                ...
                                                selector_link = 'video source[title="720p"]',
                                               )
