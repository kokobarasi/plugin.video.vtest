...
search_path_bigwank = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_bigwank.json")
mode_key  = os.path.join(xbmc.translatePath('special://home/'), keys_path + "modelist.json")
search_file_bigwank = os.path.join(xbmc.translatePath('special://home/'), search_path_bigwank + "mysearch.txt")
...
path_dicts, mode_bigwank_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'bigwankhome')]
path_dict = load_keys()
...
def readWrite_bigwank(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_bigwank,'w') as file_handle:
        ...
        with open(search_file_bigwank, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if m_part.endswith('.com/') and num > 1:
    ...
    elif not m_part.endswith('.com/') and not 'search' in m_part and num > 1:
        ...
        readWrite_bigwank(arg_str = '-'.join(name.title().split()), write=True)

        initial_url = m_part, params

    elif not m_part.endswith('.com/') and 'search' in m_part and num > 1:

        name = m_part + readWrite_bigwank() + '/' + str(num) + '/'
...
def fetch_links(m_part, min_search = 1, max_search = 2):
    ...
    movie_src = page_download.playable_links_tup(selector_list = '.thumb__top',
...
def name_site(): 
    """""" 
    page_download = WebCrawler('https://www.bigwank.com/models/')

    name_dict = page_download.fetch_names(selector='.thumb__top',
                                          image='src',
                                          title='title',
                                          arg=3,
                                          arg_num=74,
...
def cat_site(): 
    """""" 
    page_download = WebCrawler('https://www.bigwank.com/categories/')

    cat_dict = page_download.fetch_list(selector='.letter-block__link',
                                        attr='href',
                                        title = 'span',
                                        img='https://img.bigwank.com/93344000/93344333/medium@2x/1.jpg',
                                        group_dict=True
                                        )
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if   mode   == "bigwanknames":  site_name(name_site)
    elif mode   == "bigwankcat":    cat_name(cat_site)
