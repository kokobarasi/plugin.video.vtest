...
search_path_wolf = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'
search_file_wolf = os.path.join(xbmc.translatePath('special://home/'), search_path_wolf + "mysearch.txt")

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "key_wolf.json")
...
path_dicts, mode_wolf_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'wolfhome')]
path_dict = load_keys()
...
def readWrite_wolf(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_wolf,'w') as file_handle:
        ...
        with open(search_file_wolf, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if num <= 1 and not '/s/' in m_part:
        ...
        modified_part = re.sub('/s/','/search/',m_part)

        name = wolf_entry()

        params['q'] = name

        readWrite_wolf(arg_str = '+'.join(name.title().split()), write=True)
        ...
        name = modified_part + str(num) + '/?q=' + readWrite_wolf()
...
def name_site(): 
    """""" 
    page_download = WebCrawler('https://www.tubewolf.com/pornstars/')

    name_dict = page_download.fetch_list(selector='.models-list a',
                                         attr='href',
                                         title_split = True,
                                         title_split_pos = -2,
...
def cat_site(): 
    """""" 
    page_download = WebCrawler('https://www.tubewolf.com/categories/')
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if   mode   == "wolfnames":  site_name(name_site)
    elif mode   == "wolfcat":    cat_name(cat_site)
