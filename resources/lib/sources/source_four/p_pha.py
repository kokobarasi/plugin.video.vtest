...
search_path_alpha = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'
search_file_alpha = os.path.join(xbmc.translatePath('special://home/'), search_path_alpha + "mysearch.txt")

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_pha.json")
...
path_dicts, mode_pha_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'phahome')]
path_dict = load_keys()
...
def readWrite_alpha(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_alpha,'w') as file_handle:
        ...
        with open(search_file_alpha, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if m_part.endswith('.com/') and num > 1 and not 'search' in m_part:
        ...
        readWrite_alpha(arg_str = '+'.join(name.title().split()), write=True)

        initial_url = m_part, params

    elif not m_part.endswith('.com/') and 'search' in m_part and num > 1:

        name = m_part + str(num) + '/?q=' + readWrite_alpha() 
...
def name_site(): 
    """""" 
    page_download = WebCrawler('https://www.alphaporno.com/pornstars/name/')

    name_dict = page_download.fetch_names(selector='.models-list a',
                                          title_split = -2,
                                          image='src',
                                          arg = 3,
                                          arg_num = 158,
...
def cat_site(): 
    """""" 
    page_download = WebCrawler('https://www.alphaporno.com/categories/')

    cat_dict = page_download.fetch_names(selector='.cat-thumb a',
                                         image_alt = 'alt',
                                         image='src',
                                         arg = 3,
                                         arg_num = 9,
...
def channel_site(): 
    """""" 
    page_download = WebCrawler('https://www.alphaporno.com/channels/')

    name_dict = page_download.fetch_names(selector='.channel-thumb a',
                                          title = 'title',
                                          image='src',
                                          arg = 3,
                                          arg_num = 26,
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if   mode   == "phanames":  site_name(name_site)
    elif mode   == "phacat":    cat_name(cat_site)
    elif mode   == "phachannel":site_channel(channel_site)
