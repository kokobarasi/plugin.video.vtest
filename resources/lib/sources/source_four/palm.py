...
search_path = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_palmtube.json")
mode_key    = os.path.join(xbmc.translatePath('special://home/'), keys_path + "modelist.json")
search_file = os.path.join(xbmc.translatePath('special://home/'), search_path + "mysearch.txt")
...
path_dicts, mode_palm_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'palmhome')]
path_dict = load_keys()
...
def readWrite_link(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file,'w') as file_handle:
...
def url_link(m_part, num):
    ...
    if (m_part.endswith('.com/') or
        m_part.endswith('all_categories/') or 
        m_part.endswith('pornstars/')) and num > 1:
        
        initial_url = m_part + '?page=' + str(num)

    elif m_part.endswith('s=1') or m_part.endswith('s=2') or m_part.endswith('s=3') or m_part.endswith('s=4') and num > 1: 

        initial_url = m_part + '&page=' + str(num)

    elif  m_part.endswith('/channel/') and num <= 1:

        name = palm_entry()

        params['q'] = name

        readWrite_link(arg_str = m_part+'?q='+name, write=True)

        initial_url = m_part, params


    elif  '/channel/' in m_part and num > 1:

        name = readWrite_link()

        initial_url = name + '&page=' + str(2)
...
def fetch_links(m_part, min_search = 1, max_search = 2):
    ...
    movie_src = page_download.playable_links_tup(selector_list = 'a[href*=watch]',
...
def name_site(): 
    """""" 
    page_download = WebCrawler('https://palmtube.com/pornstars/')

    name_dict = page_download.fetch_names(selector='.Thumb_Category a',
                                          image='data-src',
                                          image_alt='alt',
                                          arg=3,
                                          arg_num=10,
...
def cat_site(): 
    """""" 
    page_download = WebCrawler('https://palmtube.com/all_categories/?page=')

    cat_dict = page_download.fetch_cat(link_selector = '.Thumb_Category a',
                                      iteration = list(range(1,3)),
                                      str_selector='.Thumb_Category a .Thumb_Title',
                                      img_selector='.Thumb_Category a img',
                                      src='data-src'
                                      )
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if   mode   == "palmnames":  site_name(name_site)
    elif mode   == "palmcat":    cat_name(cat_site)
