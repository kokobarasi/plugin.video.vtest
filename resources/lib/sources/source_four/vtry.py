...
search_path_vtry = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'
search_file_vtry = os.path.join(xbmc.translatePath('special://home/'), search_path_vtry + "mysearch.txt")

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_vtry.json")
...
path_dicts, mode_vtry_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'vtryhome')]
path_dict = load_keys()
...
def readWrite_vtry(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_vtry,'w') as file_handle:
        ...
        with open(search_file_vtry, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if m_part.endswith('.com/') and num <= 1 and not 'search' in m_part:
    ...
    elif '/categories/' in m_part or '/models/' in m_part and num <=1 and not 'search' in m_part:
        ...
        readWrite_vtry(arg_str = '-'.join(name.lower().split()), write=True)

        initial_url = m_part, params

    elif 'search' in m_part and num > 1:

        name = m_part + readWrite_vtry() + '/' + str(num) + '/'
...
def fetch_links(m_part, min_search = 1, max_search = 2):
    ...
    movie_src = page_download.playable_links_tup(selector_list = '.thumb a',
                             search_pattern = '(\s-\s\w+\.\w{3})',
                             replace_string = '',
                             element = 'title',
                             img_selector = 'video',
...
def cat_site():
    """""" 
    page_download = WebCrawler('https://www.veryfreeporn.com/categories/')

    cat_dict = page_download.fetch_list(selector='.letter-block__item a',
                                   attr='href',
                                   title='span',
                                   img='https://img.veryfreeporn.com/12496000/12496215/medium@2x/1.jpg',
                                   attr_option = '/www.veryfreeporn.com/',
                                   group_dict = True
                                   )
...
def name_site(): 
    """""" 
    page_download = WebCrawler('https://www.veryfreeporn.com/models/')

    name_dict = page_download.fetch_names(selector='.thumb a', 
                                     title='title', 
                                     arg = 3, arg_num = 95, 
                                     query_string='/'
                                     )
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    mode = re.compile(r"mode=(\w+)?&").search(args).group(1)
    if   mode   == "vtrycat":     site_cat(cat_site)
    elif mode   == "vtryname":    site_name(name_site)
