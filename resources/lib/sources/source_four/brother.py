...
dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_brother.json")
...
path_dicts, mode_brother_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'brotherhome')]
path_dict = load_keys()


def url_link(m_part, num):

    if num <= 1 and m_part.endswith('/'): 

        initial_url = m_part

    elif num > 1 and m_part.endswith('/'):
...
def fetch_links(m_part, min_search = 1, max_search = 2):
    ...
    movie_src = page_download.playable_links_tup(selector_list = '.postbox a',
                                                 ...
                                                 attr_option = 'https://tubebrother.com/',
...
def channel():
    """""" 
    page_download = WebCrawler('https://tubebrother.com/porn-channels/')
    
    name_dict = page_download.fetch_list(selector='.pciwgas-img-wrapper a',
                                         attr='href',
                                         title_split=True,
                                         title_split_pos = 4,
                                         img='data-lazy-src',
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if   mode   == "brotherchannel":    site_channel(channel)
