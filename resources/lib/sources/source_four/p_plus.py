...
search_path_plus = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'
search_file_plus = os.path.join(xbmc.translatePath('special://home/'), search_path_plus + "mysearch.txt")

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_plus.json")
...
path_dicts, mode_plus_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'plushome')]
path_dict = load_keys()
...
def readWrite_plus(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_plus,'w') as file_handle:
        ...
        with open(search_file_plus, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if num > 1 and not '.com/s/' in m_part:
        
        initial_url = m_part + str(num) + '/'

    elif  m_part.endswith('.com/s/') and num <= 1:

        name = plus_entry()

        params['q'] = name

        readWrite_plus(arg_str = '+'.join(name.title().split()), write=True)

        initial_url = m_part, params

    elif '.com/s/' in m_part and num > 1:

        name = m_part + str(num) + '/?q=' + readWrite_plus()
...
def name_site(): 
    """""" 
    page_download = WebCrawler('https://www.porn-plus.com/p/')

    name_dict = page_download.fetch_list(selector='.tagslist a',
                                         attr='href',
                                         attr_option = '/p/',
                                         title_split = True,
                                         title_split_pos = -2,
                                         img = 'https://static-pp2.cdnanp.com/contents/models/36/s1_alexis-texas-1.jpg',
                                         group_dict = True
                                         )
...
def cat_site(): 
    """""" 
    page_download = WebCrawler('https://www.porn-plus.com/c/')

    cat_dict = page_download.fetch_list(selector='.thmb a',
                                        attr='href',
                                        attr_option = '/c/',
                                        title = 'title',
                                        img = 'src',
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if   mode   == "plusnames":  site_name(name_site)
    elif mode   == "pluscat":    cat_name(cat_site)
