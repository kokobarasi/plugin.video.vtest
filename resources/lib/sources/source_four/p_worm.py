...
search_path_worm = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'
search_file_worm = os.path.join(xbmc.translatePath('special://home/'), search_path_worm + "mysearch.txt")

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_worm.json")
...
path_dicts, mode_worm_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'wormhome')]
path_dict = load_keys()
...
def readWrite_worm(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_worm,'w') as file_handle:
        ...
        with open(search_file_worm, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if num > 1 and not '/video/' in m_part:
        
        initial_url = m_part + str(num) + '/'

    elif  m_part.endswith('/video/') and num <= 1:

        name = worm_entry()

        params['s'] = name

        readWrite_worm(arg_str = '%20'.join(name.title().split()), write=True)

        initial_url = m_part, params

    elif '/video/' in m_part and num > 1:

        name = m_part + '?s=' + readWrite_worm() + '&page=' + str(num)
...
def name_site(): 
    """""" 
    page_download = WebCrawler('https://www.pornworms.com/pornstar/')

    name_dict = page_download.fetch_names(selector='.thumbnail', 
                                          title = 'title',
                                          image='src',
                                          arg = 3,
                                          arg_num = 330,
...
def cat_site(): 
    """""" 
    page_download = WebCrawler('https://www.pornworms.com/categories/')

    cat_dict = page_download.fetch_list(selector='.thumbnail',
                                        attr='href',
                                        title_split=True,
                                        title_split_pos = -2,
                                        img = 'src',
...
def channel_site(): 
    """""" 
    page_download = WebCrawler('https://www.pornworms.com/channels/name/')

    cat_dict = page_download.fetch_names(selector='.thumbnail',
                                         image_alt = 'alt',
                                         image='src',
                                         arg = 3,
                                         arg_num = 23,
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if   mode   == "wormnames":  site_name(name_site)
    elif mode   == "wormcat":    cat_name(cat_site)
    elif mode   == "woemkcat":    channel_name(channel_site)
