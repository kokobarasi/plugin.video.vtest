...
search_path_perv = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_perv.json")
mode_key  = os.path.join(xbmc.translatePath('special://home/'), keys_path + "modelist.json")
search_file_perv = os.path.join(xbmc.translatePath('special://home/'), search_path_perv + "mysearch.txt")
...
path_dicts, mode_perv_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'pervhome')]
path_dict = load_keys()
...
def readWrite_perv(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_perv,'w') as file_handle:
        ...
        with open(search_file_perv, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if m_part.endswith('.com/'):
        ...
        if not '/search/' in m_part and num <= 1:
            ...
            initial_url = m_part + 'tube/latest-updates/' + str(num) + '/'
        ...
        elif  m_part.endswith('/search/') and num <= 1:#?q=alexis+texas

            name = perv_entry()

            params['q'] = name
                
            readWrite_perv(arg_str = '+'.join(name.lower().split()), write=True)
                
            initial_url = m_part, params

        elif m_part.endswith('/search/') and num > 1:

            name = m_part + '?q=' + readWrite_perv()
...
def cat_site(): 
    """""" 
    page_download = WebCrawler('https://www.pervclips.com/tube/categories/')
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if   mode   == "pervcat"   :  cat_name(cat_site)
