...
search_path_psex = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_anysex.json")
mode_key  = os.path.join(xbmc.translatePath('special://home/'), keys_path + "modelist.json")
search_file_psex = os.path.join(xbmc.translatePath('special://home/'), search_path_psex + "mysearch.txt")
...
path_dicts, mode_anysex_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'anysexhome')]
path_dict = load_keys()
...
def readWrite_psex(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_psex,'w') as file_handle:
        ...
        with open(search_file_psex, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if m_part.endswith('.com/') and not 'search' in m_part and num > 1:
        
        initial_url = m_part + 'new-movies/' + str(num) + '/'
        ...
        readWrite_psex(arg_str = '+'.join(name.lower().split()), write=True)

        initial_url = m_part, params

    elif not m_part.endswith('.com/') and 'search' in m_part and num > 1:

        name = m_part + str(num)+ '/?q=' + readWrite_psex()
...
def fetch_links(m_part, min_search = 1, max_search = 2):
    ...
    movie_src = page_download.playable_links_tup(selector_list = '.popfire',
...
def name_site(): 
    """""" 
    page_download = WebCrawler('https://anysex.com/models/')
...
def cat_site(): 
    """""" 
    page_download = WebCrawler('https://anysex.com/categories/')

    cat_dict = page_download.fetch_list(selector='.popfire',
                                        attr='href',
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if   mode   == "anysexname"   :  site_name(name_site)
    elif mode   == "anysexcat"    :    cat_name(cat_site)
    elif mode   == "anysexnamefilter"    : site_name_filter(name_site_filter)
