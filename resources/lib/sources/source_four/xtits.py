...
search_path_xtits = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_xtits.json")
mode_key  = os.path.join(xbmc.translatePath('special://home/'), keys_path + "modelist.json")
search_file_xtits = os.path.join(xbmc.translatePath('special://home/'), search_path_xtits + "mysearch.txt")
...
path_dicts, mode_xtits_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'xtitshome')]
path_dict = load_keys()
...
def readWrite_xtits(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_xtits,'w') as file_handle:
        ...
        with open(search_file_xtits, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if m_part.endswith('.com/'):
        ...
        if not '/search/' in m_part and num <= 1:
            ...
            readWrite_xtits(arg_str = '-'.join(name.lower().split()), write=True)
                
            initial_url = m_part, params

        elif m_part.endswith('/search/') and num > 1:

            name = m_part + readWrite_xtits() + '?from='  + str(num)
...
def name_site(): 
    """""" 
    page_download = WebCrawler('https://www.xtits.xxx/models/')
...
def cat_site(): 
    """""" 
    page_download = WebCrawler('https://www.xtits.xxx/categories/')
...
def channel_site(): 
    """""" 
    page_download = WebCrawler('https://www.xtits.xxx/sites/')
...
def playlist_site(): 
    """""" 
    page_download = WebCrawler('https://www.xtits.xxx/playlists/')
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if   mode   == "xtitsname":  site_name(name_site)
    elif mode   == "xtitscat":    cat_name(cat_site)
    elif mode   == "xtitstopname"  :  site_name_filter(name_site_filter)
    elif mode   == "xtitsplaylist":  playlist_name(playlist_site)
    elif mode   == "xtitschannel":    channel_name(channel_site)
