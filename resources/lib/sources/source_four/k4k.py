...
search_path_k4k = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_k4k.json")
mode_key  = os.path.join(xbmc.translatePath('special://home/'), keys_path + "modelist.json")
search_file_k4k = os.path.join(xbmc.translatePath('special://home/'), search_path_k4k + "mysearch.txt")
...
path_dicts, mode_k4k_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'k4khome')]
path_dict = load_keys()
...
def readWrite_k4k(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_k4k,'w') as file_handle:
        ...
        with open(search_file_k4k, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if m_part.endswith('.xxx/'):
        ...
        if not '/search/' in m_part and num <= 1:
            ...
            readWrite_k4k(arg_str = '-'.join(name.lower().split()), write=True)
                
            initial_url = m_part, params

        elif m_part.endswith('/search/') and num > 1:

            name = m_part + readWrite_k4k() + '/?from_videos='  + str(num)
...
def name_site(): 
    """""" 
    page_download = WebCrawler('https://4kporn.xxx/models/')
...
def cat_site(): 
    """""" 
    page_download = WebCrawler('https://4kporn.xxx/categories/')
...
def channel_site(): 
    """""" 
    page_download = WebCrawler('https://4kporn.xxx/sites/')
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if   mode   == "k4kname"      :  site_name(name_site)
    elif mode   == "k4kchannel"   :  channel_name(channel_site)
    elif mode   == "k4kcat"       :  cat_name(cat_site)
    elif mode   == "k4ktopname"  :  site_name_filter(name_site_filter)
