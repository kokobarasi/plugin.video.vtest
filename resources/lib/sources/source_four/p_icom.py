...
search_path_icom = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_icom.json")
mode_key  = os.path.join(xbmc.translatePath('special://home/'), keys_path + "modelist.json")
search_file_icom = os.path.join(xbmc.translatePath('special://home/'), search_path_icom + "mysearch.txt")
...
path_dicts, mode_icom_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'icomhome')]
path_dict = load_keys()
...
def readWrite_icom(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_icom,'w') as file_handle:
        ...
        with open(search_file_icom, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if m_part.endswith('.com/') and num > 1 and not 'search' in m_part:
        ...
        readWrite_icom(arg_str = '+'.join(name.lower().split()), write=True)

        initial_url = m_part, params

    elif not m_part.endswith('.com/') and 'search' in m_part and num > 1:

        name = m_part + str(num) + '/' + '?q=' + readWrite_icom()
...
def cat_site(): 
    """""" 
    page_download = WebCrawler("https://www.pornicom.com/categories/")

    cat_dict = page_download.fetch_list(selector='.link-thumb',
                                        attr='href',
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if mode   == "icomcat":    cat_name(cat_site)
