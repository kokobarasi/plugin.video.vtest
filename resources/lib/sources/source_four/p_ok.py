...
search_path_ok = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'
search_file_ok = os.path.join(xbmc.translatePath('special://home/'), search_path_ok + "mysearch.txt")

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_ok.json")
...
path_dicts, mode_ok_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'okhome')]
path_dict = load_keys()
...
def readWrite_ok(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_ok,'w') as file_handle:
        ...
        with open(search_file_ok, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if num <= 1 and not 'search' in m_part: initial_url = m_part

    elif  num > 1 and not 'search' in m_part: initial_url = m_part + str(num) + '/'
        ...
        readWrite_ok(arg_str = '-'.join(name.lower().split()), write=True)

        initial_url = m_part, params

    elif 'search' in m_part and num > 1:

        name = m_part + readWrite_ok() + '/' + str(num) + '/'
...
def name_site(): 
    """""" 
    page_download = WebCrawler('https://ok.porn/models/')
...
def channel_site(): 
    """""" 
    page_download = WebCrawler('https://ok.porn/channels/')

    cat_dict = page_download.fetch_list(selector='.thumb a',
                                  attr='href',
                                  title='title',
                                  img='src',
                                  attr_option = '/sites/',
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    mode = re.compile(r"mode=(\w+)?&").search(args).group(1)
    if   mode   == "oknames":      site_name(name_site)
    elif mode   == "okchannel":    channel_name(channel_site)
