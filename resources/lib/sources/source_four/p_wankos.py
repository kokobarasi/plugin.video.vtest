...
search_path_wankoz = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'
search_file_wankoz = os.path.join(xbmc.translatePath('special://home/'), search_path_wankoz + "mysearch.txt")

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_wankoz.json")
...
path_dicts, mode_wankos_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'wankozhome')]
path_dict = load_keys()
...
def readWrite_wankoz(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_wankoz,'w') as file_handle:
        ...
        with open(search_file_wankoz, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if m_part.endswith('.com/') and num > 1 and not '/search/' in m_part:
        ...
        readWrite_wankoz(arg_str = '+'.join(name.title().split()), write=True)

        initial_url = m_part, params

    elif '/search/' in m_part and num > 1:

        name = m_part + str(num) + '/?q=' + readWrite_wankoz()
...
def name_site(): 
    """""" 
    page_download = WebCrawler('https://www.wankoz.com/pornstars/')

    name_dict = page_download.fetch_names(selector='.thumbs-list .thumb a', 
                                          title = 'title',
                                          image='src',
                                          arg = 3,
                                          arg_num = 42,
...
def cat_site(): 
    """""" 
    page_download = WebCrawler('https://www.wankoz.com/categories/')

    cat_dict = page_download.fetch_list(selector='.thumbs-list .thumb a',
                                        attr='href',
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if   mode   == "wankoznames":  site_name(name_site)
    elif mode   == "wankozcat":    cat_name(cat_site)
