...
search_path_shake = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_shake.json")
mode_key  = os.path.join(xbmc.translatePath('special://home/'), keys_path + "modelist.json")
search_file_shake = os.path.join(xbmc.translatePath('special://home/'), search_path_shake + "mysearch.txt")
...
path_dicts, mode_shake_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'shakehome')]
path_dict = load_keys()
...
def readWrite_shake(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_shake,'w') as file_handle:
        ...
        with open(search_file_shake, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if m_part.endswith('.com/'):
        ...
        if not '/search/' in m_part and num <= 1:
            ...
            readWrite_shake(arg_str = '-'.join(name.lower().split()), write=True)
                
            initial_url = m_part, params

        elif m_part.endswith('/search/') and num > 1:

            name = m_part + readWrite_shake() + '/' + str(num) + '/'
...
def name_site(): 
    """""" 
    page_download = WebCrawler('https://xxxshake.com/pornstars/')

    name_dict = page_download.fetch_names(
                                    selector = '#list_models_models_list_items .item',
                                    image='src', 
                                    title = "title",
                                    arg = 3,
                                    arg_num = 176,
...
def cat_site(): 
    """""" 
    page_download = WebCrawler('https://xxxshake.com/tags/')

    cat_dict = page_download.fetch_list(

                                  selector='#list_categories_categories_list_items a',
                                  attr='href',
                                  attr_option = '/tags/',
                                  title = 'title',
                                  img = 'src',
...
def channel_site(): 
    """""" 
    page_download = WebCrawler('https://xxxshake.com/channels/')

    cat_dict = page_download.fetch_names(
                                    selector = '#list_content_sources_sponsors_list_items .item',
                                    image='src', 
                                    title = "title",
                                    arg = 3,
                                    arg_num = 68,
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if   mode   == "shakename"      :  site_name(name_site)
    elif mode   == "shakechannel"   :  channel_name(channel_site)
    elif mode   == "shakecat"       :  cat_name(cat_site)
    elif mode   == "shaketopname"  :  site_name_filter(name_site_filter)
