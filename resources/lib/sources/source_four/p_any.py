...
search_path_anyp = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'
search_file_anyp = os.path.join(xbmc.translatePath('special://home/'), search_path_anyp + "mysearch.txt")

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_anyporn.json")
...
path_dicts, mode_any_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'anyhome')]
path_dict = load_keys()
...
def readWrite_anyp(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_anyp,'w') as file_handle:
        ...
        with open(search_file_anyp, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if not '/search/' in m_part:
        ...
        readWrite_anyp(m_part + '-'.join(name.split()) + '/', write=True)

        initial_url = m_part, params

    elif '/search/' in m_part and num > 1:

        name = readWrite_anyp() + '?from=' + str(num)
...
def channel_site(): 
    """""" 
    page_download = WebCrawler('https://anyporn.com/channels/')
...
def cat_site(): 
    """""" 
    page_download = WebCrawler('https://anyporn.com/categories/')

    cat_dict = page_download.fetch_list(selector='.item',
                                        attr='href',
...
def catAll_site(): 
    ...
    cat_dict = page_download.fetch_list(selector='.tagslist a',
                                        attr='href',
                                        title='title',
                                        img='https://static-ap.cdnanp.com/categories/60.jpg',
                                        group_dict = True
                                        )
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if   mode   == "anychannel":  site_channel(channel_site)
    elif mode   == "anycat"    :    cat_name(cat_site)
    elif mode   == "anycatall" :    catAll_name(catAll_site)
