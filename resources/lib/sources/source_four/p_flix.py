...
search_path_pflix = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_pfeat.json")
mode_key  = os.path.join(xbmc.translatePath('special://home/'), keys_path + "modelist.json")
search_file_pflix = os.path.join(xbmc.translatePath('special://home/'), search_path_pflix + "mysearch.txt")
...
path_dicts, mode_pflix_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'pfeathome')]
path_dict = load_keys()
...
def readWrite_pflix(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_pflix,'w') as file_handle:
        ...
        with open(search_file_pflix, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if not '/search/' in m_part: 
        ...
        elif num > 1: initial_url = m_part + 'page/' + str(num) + '/'
        ...
        readWrite_pflix('https://www.pornfeat.com/' + '?post_type=video_skrn&search_keyword=' + '+'.join(name.split()) + '/', write=True)

        initial_url = m_part, params

    elif '/search/' in m_part and num > 1:

        name = readWrite_pflix() + '/page/' + str(num) + '/'
...
def fetch_links(m_part, min_search = 1, max_search = 2):
    ...
    if len(f_url) == 2: 
            ...
            readWrite_pflix(arg_str = page_download.html(url=True), write=True)
    ...
    movie_src = page_download.playable_links_tup(
                                                selector_list='.elementor-section a',
...
def cat_site(): 
    """""" 
    page_download = WebCrawler('https://www.pornfeat.com/categories/')

    cat_dict = page_download.fetch_names(
                                         selector = '.progression-studios-video-index a',
                                         image='src', 
                                         title_split = -2,
                                         arg = 3,
                                         arg_num = 2,
...
def channel_site(): 
    """""" 
    page_download = WebCrawler('https://www.pornfeat.com/channels/')
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if   mode   == "pflixname":      site_name(name_site)
    elif mode   == "pflixcat":        cat_name(cat_site)
    elif mode   == "pflixchannel":    channel_name(channel_site)
