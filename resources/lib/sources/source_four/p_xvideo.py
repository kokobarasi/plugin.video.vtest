...
search_path_pxvideo = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_xvideos.json")
mode_key  = os.path.join(xbmc.translatePath('special://home/'), keys_path + "modelist.json")
search_file_pxvideo = os.path.join(xbmc.translatePath('special://home/'), search_path_pxvideo + "mysearch.txt")
...
path_dicts, mode_xvideo_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'xvideoshome')]
path_dict = load_keys()
...
def readWrite_pxvideo(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_pxvideo,'w') as file_handle:
        ...
        with open(search_file_pxvideo, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if num > 1 and not 'search' in m_part:
        ...
        initial_url = m_part + 'new/' + str(num) + '/'

    elif num <= 1 and not 'search' in m_part:

        initial_url = m_part

    elif 'k=PAMEla+rios' in m_part and not 'search' in m_part and num <=1:
        ...
        initial_url = m_part + '&p=' + str(update_num)
        ...
        params['k'] = name
        _m_part = re.sub('/search/','/',m_part)

        readWrite_pxvideo(arg_str = '+'.join(name.lower().split()), write=True)

        initial_url = _m_part, params

    elif 'search' in m_part and num > 1:

        _m_part = re.sub('/search/','/',m_part)

        name = _m_part + '?k=' + readWrite_pxvideo() + '&p=' + str(num - 1)
...
def name_site(): 
    """""" 
    page_download = WebCrawler('https://www.xvideos.com/pornstars-index/')

    name_dict =  page_download.fetch_names(selector='.thumb-under a', 
                                           title_split = -1,
                                           image='https://img-l3.xvideos-cdn.com/videos_new/thumbs169ll/24/b2/c6/24b2c6d31fe440e36d39cbadbb3172c8/24b2c6d31fe440e36d39cbadbb3172c8.12.jpg',
                                           arg = 3,
                                           arg_num = 345,
                                           query_string='/',
                                           index=True
...
def channel_site(): 
    """""" 
    page_download = WebCrawler('https://www.xvideos.com/channels-index/')

    cat_dict = page_download.fetch_names(selector='.thumb a', 
                                         title_split = -1,
                                         image='https://img-l3.xvideos-cdn.com/videos_new/thumbs169ll/24/b2/c6/24b2c6d31fe440e36d39cbadbb3172c8/24b2c6d31fe440e36d39cbadbb3172c8.12.jpg',
                                         arg = 3,
                                         arg_num = 161,
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if   mode   == "xvideosnames":  site_name(name_site)
    elif mode   == "xvideoschannel":    channel_name(channel_site)
