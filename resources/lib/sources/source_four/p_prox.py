...
search_path_prox = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'
search_file_prox = os.path.join(xbmc.translatePath('special://home/'), search_path_prox + "mysearch.txt")

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_xozilla.json")
...
path_dicts, mode_prox_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'xozillahome')]
path_dict = load_keys()
...
def readWrite_prox(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_prox,'w') as file_handle:
        ...
        with open(search_file_prox, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if not '/search/' in m_part:
        ...
        readWrite_prox(m_part + '-'.join(name.split()) + '/', write=True)
        ...
        name = readWrite_prox() + '?from=' + str(num)
...
def fetch_links(m_part, min_search = 1, max_search = 2):
    ...
    movie_src = page_download.playable_links_tup(selector_list = '.item',
...
def name_site(): 
    """""" 
    page_download = WebCrawler('https://www.xozilla.com/models/')
...
def cat_site(): 
    """""" 
    page_download = WebCrawler('https://www.xozilla.com/categories/')
    ...
    cat_dict = page_download.fetch_list(
                                        ...
                                        title_split = True,
                                        title_split_pos = 4,
                                        img = 'https://i.xozilla.com/contents/categories/687/687.jpg',
                                        group_dict = 'True'
                                        )
...
def channel_site(): 
    """""" 
    page_download = WebCrawler('https://www.xozilla.com/channels/')
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if   mode   == "xozillaname":       site_name(name_site)
    elif mode   == "xozillacat":        cat_name(cat_site)
    elif mode   == "xozillaallcat":     a_cat_name(a_cat_site)
    elif mode   == "xozillachannel":    channel_name(channel_site)
