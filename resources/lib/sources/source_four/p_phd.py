...
search_path_phd = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'
search_file_phd = os.path.join(xbmc.translatePath('special://home/'), search_path_phd + "mysearch.txt")

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_analdin.json")
...
path_dicts, mode_phd_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'analdinhome')]
path_dict = load_keys()
...
def readWrite_phd(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_phd,'w') as file_handle:
        ...
        with open(search_file_phd, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if not '/search/' in m_part:

        if num <= 1:

                initial_url = m_part
                xbmc.log("*"*100)
                xbmc.log(str(initial_url))
                xbmc.log("*"*100)
        ...
        readWrite_phd(m_part + '-'.join(name.split()) + '/', write=True)
        ...
        name = readWrite_phd() + '?from=' + str(num)
...
def fetch_links(m_part, min_search = 1, max_search = 2):
    ...
    try:
            movie_src = page_download.play_links_tup(
                                       selector_list = '.popup-video-link',
                                       img_selector = 'data-original',
                                       title_split = True,
                                       title_split_pos = 5,
                                       attr_option = '/videos/',
                                       sieve_str = True,
                                       sieve_pattern = "video_url:\s+'(.*?\.mp4/)",
                                       meta_desc = 'meta[name="keywords"]', 
                                    )

    except AttributeError:

            movie_src = page_download.play_links_tup(
                                       selector_list = '.playlist-holder .item',
                                       img_selector = 'data-original',
                                       title = 'alt',
                                       attr_option = '/videos/',
                                       sieve_str = True,
...
def cat_site(arg_num=0): 
    """""" 
    page_download = WebCrawler('https://www.analdin.com/categories/')
    ...
    if arg_num == 0:
            ...
            cat_dict = page_download.fetch_list(
                                      ...
                                      title_split=True,
                                      title_split_pos = 4,
                                      img = 'https://i.analdin.com/contents/categories/290.jpg',
                                      group_dict = 'True'
...
def cat_name(arg, arg_num=0):
    """""" 
    name_dict = arg(arg_num)
...
def channel_site(): 
    """""" 
    page_download = WebCrawler('https://www.analdin.com/')
            
    cat_dict = page_download.fetch_list(

                                      selector='.popup-categories-list a',
                                      attr='href',
...
def model_site(): 
    """""" 
    page_download = WebCrawler('https://www.analdin.com/models/')
...
def playlist_site(): 
    """""" 
    page_download = WebCrawler('https://www.analdin.com/playlists/')
            
    cat_dict = page_download.fetch_names(
                                    selector = 'a',
                                    attr_option = '/playlist/',
                                    image='data-original',
                                    title = 'title',
                                    arg = 3,
                                    arg_num = 55,
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if mode     == "analdincat":        cat_name(cat_site)
    elif mode   == "analdincatAll":     cat_name(cat_site, 1)
    elif mode   == "analdinchannel":    channel_name(channel_site)
    elif mode   == "analdinmodels":    model_name(model_site)
    elif mode   == "analdinplaylist":  playlist_name(playlist_site)
