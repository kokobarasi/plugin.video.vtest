...
search_path_iporn = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'
dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_iporn.json")
search_file_iporn = os.path.join(xbmc.translatePath('special://home/'), search_path_iporn + "mysearch.txt")
...
path_dicts, mode_iporn_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'ipornhome')]
path_dict = load_keys()
...
def readWrite_iporn(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_iporn,'w') as file_handle:
        ...
        with open(search_file_iporn, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if m_part.endswith('.com/'):
        ...
        if not '/search/' in m_part and num <= 1:
            ...
            readWrite_iporn(arg_str = '-'.join(name.lower().split()), write=True)
            ...
            name = m_part + readWrite_iporn() + '/page' + str(num) + '.html' 
...
def name_site(): 
    """""" 
    page_download = WebCrawler('https://iporntoo.com/models/')
...
def cat_site(): 
    """""" 
    page_download = WebCrawler('https://iporntoo.com/channels/')
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if   mode   == "ipornname":  site_name(name_site)
    elif mode   == "iporncat":    cat_name(cat_site)
    elif mode   == "iporntopname"  :  site_name_filter(name_site_filter)
