...
search_path_lust = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_lust.json")
mode_key  = os.path.join(xbmc.translatePath('special://home/'), keys_path + "modelist.json")
search_file_lust = os.path.join(xbmc.translatePath('special://home/'), search_path_lust + "mysearch.txt")
...
path_dicts, mode_lust_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'lusthome')]
path_dict = load_keys()
...
def readWrite_lust(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_lust,'w') as file_handle:
        ...
        with open(search_file_lust, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if m_part.endswith('.com/'):
        ...
        if not '/search/' in m_part and num <= 1:
            ...
            readWrite_lust(arg_str = '+'.join(name.lower().split()), write=True)
                
            initial_url = m_part, params

        elif m_part.endswith('/search/') and num > 1:

            name = m_part + '/?q=' + readWrite_lust() +  '/' + str(num) + '/' 
...
def cat_site(): 
    """""" 
    page_download = WebCrawler('https://mylust.com/categories/')

    cat_dict = page_download.fetch_list(

                                  selector='.list_categories a',
                                  attr='href',
...
def channel_site(): 
    """""" 
    page_download = WebCrawler('https://mylust.com/channels/')
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if mode     == "lustchannel"   :  channel_name(channel_site)
    elif mode   == "lustcat"       :  cat_name(cat_site)
