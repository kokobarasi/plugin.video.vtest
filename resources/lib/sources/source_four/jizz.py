...
search_path_jizz = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_jizz.json")
mode_key  = os.path.join(xbmc.translatePath('special://home/'), keys_path + "modelist.json")
search_file_jizz = os.path.join(xbmc.translatePath('special://home/'), search_path_jizz + "mysearch.txt")
...
path_dicts, mode_jizz_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'jizzhome')]
path_dict = load_keys()
...
def readWrite_jizz(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_jizz,'w') as file_handle:
        ...
        with open(search_file_jizz, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if m_part.endswith('.com/'):
        ...
        if not '/search/' in m_part and num <= 1:
            ...
            initial_url = m_part + 'newest/' + str(num) + '/'
        ...
        if not '/search/' in m_part and num <= 1:
            ...
            m_part_ = re.sub('search','s',m_part)
            readWrite_jizz(arg_str = '-'.join(name.lower().split()), write=True)
            ...
            m_part_ = re.sub('search','s',m_part)
            name = m_part_ + readWrite_jizz() + '/' + str(num) + '/'
...
def name_site(): 
    """""" 
    page_download = WebCrawler('https://jizzberry.com/porn-models/')
...
def cat_site(): 
    """""" 
    page_download = WebCrawler('https://jizzberry.com/categories/')
...
def channel_site(): 
    """""" 
    page_download = WebCrawler('https://jizzberry.com/channels/')
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if   mode   == "jizzname"      :  site_name(name_site)
    elif mode   == "jizzchannel"   :  channel_name(channel_site)
    elif mode   == "jizzcat"       :  cat_name(cat_site)
    elif mode   == "jizztopname"  :  site_name_filter(name_site_filter)
