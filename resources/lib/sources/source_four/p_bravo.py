...
search_path_bravo = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'
search_file_bravo = os.path.join(xbmc.translatePath('special://home/'), search_path_bravo + "mysearch.txt")

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_bravo.json")
...
path_dicts, mode_bravo_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'bravohome')]
path_dict = load_keys()
...
def readWrite_bravo(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_bravo,'w') as file_handle:
        ...
        with open(search_file_bravo, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if m_part.endswith('.com/') and num > 1 and not '/s/' in m_part:
        
        initial_url = m_part + 'latest-updates/' + str(num) + '/'

    elif num > 1 and not '/s/' in m_part:

        initial_url = m_part + str(num) + '/'


    elif  m_part.endswith('/s/') and num <= 1:

        name = bravo_entry()

        params['q'] = name

        readWrite_bravo(arg_str = '+'.join(name.lower().split()), write=True)

        initial_url = m_part, params

    elif not m_part.endswith('.com/') and '/s/' in m_part and num > 1:

        name = m_part + str(num) + '/?q=' + readWrite_bravo() 
...
def name_site(): 
    """""" 
    page_download = WebCrawler('https://www.bravoporn.com/models/')

    name_dict = page_download.fetch_list(selector='.th',
                                         attr='href',
                                         title = 'title',
                                         attr_option = '/models/',
                                         img = 'src',
...
def cat_site(): 
    """""" 
    page_download = WebCrawler('https://www.bravoporn.com/c/')

    cat_dict = page_download.fetch_list(selector='.item a',
                                        attr='href',
                                        title = 'title',
                                        attr_option = '/c/',
                                        img = 'https://static-bp2.cdnbm.net/921000/921835/240x180/1.jpg',
                                        group_dict = True
                                        )
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if   mode   == "bravonames":  site_name(name_site)
    elif mode   == "bravocat":    cat_name(cat_site)
