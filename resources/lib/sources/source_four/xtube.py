...
search_path_xtube = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_xtube.json")
mode_key  = os.path.join(xbmc.translatePath('special://home/'), keys_path + "modelist.json")
search_file_xtube = os.path.join(xbmc.translatePath('special://home/'), search_path_xtube + "mysearch.txt")
...
path_dicts, mode_xtube_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'xtubehome')]
path_dict = load_keys()
...
def readWrite_xtube(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_xtube,'w') as file_handle:
        ...
        with open(search_file_xtube, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if m_part.endswith('.com/'):
        ...
        if not '/search/' in m_part and num <= 1:
            ...
            readWrite_xtube(arg_str = '%20'.join(name.lower().split()), write=True)
                
            initial_url = m_part, params

        elif m_part.endswith('/search/') and num > 1:

            name = m_part + readWrite_xtube() + '/page'  + str(num) + '.html'
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if   mode   == "xtubename"      :  site_name(name_site)
    elif mode   == "xtubechannel"   :  channel_name(channel_site)
    elif mode   == "xtubetopname"  :  site_name_filter(name_site_filter)
