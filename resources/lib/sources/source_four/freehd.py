...
search_path_freehd = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'
dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_freehd.json")
search_file_freehd = os.path.join(xbmc.translatePath('special://home/'), search_path_freehd + "mysearch.txt")
...
path_dicts, mode_freehd_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'freehdhome')]
path_dict = load_keys()
...
def readWrite_freehd(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_freehd,'w') as file_handle:
        ...
        with open(search_file_freehd, 'r') as fh:
            
            content = fh.read()
            
            return content

#https://sextubefun.com/page2.html
def url_link(m_part, num):
    ...
    if m_part.endswith('.com/'):
        ...
        if not '/search/' in m_part and num <= 1:
            ...
            initial_url = m_part + 'page' + str(num) + '.html'
        ...
        if not '/search/' in m_part and num <= 1:
            ...
            readWrite_freehd(arg_str = '-'.join(name.lower().split()), write=True)
                
            m_part_ = re.sub('search','searchgate.php',m_part) 
                
            initial_url = m_part_, params

        elif m_part.endswith('/search/') and num > 1:

            name = m_part + readWrite_freehd() + '/page' + str(num) + '.html' 
...
def name_site(): 
    """""" 
    page_download = WebCrawler('https://freehdporn.xxx/models/')
...
def cat_site(): 
    """""" 
    page_download = WebCrawler('https://freehdporn.xxx/channels/')
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if   mode   == "freehdname":  site_name(name_site)
    elif mode   == "freehdcat":    cat_name(cat_site)
    elif mode   == "freehdtopname"  :  site_name_filter(name_site_filter)
