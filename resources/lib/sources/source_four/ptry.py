...
search_path_ptry = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'
search_file_ptry = os.path.join(xbmc.translatePath('special://home/'), search_path_ptry + "mysearch.txt")

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_ptry.json")
...
path_dicts, mode_ptry_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'ptryhome')]
path_dict = load_keys()
...
def readWrite_ptry(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_ptry,'w') as file_handle:
        ...
        with open(search_file_ptry, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if m_part.endswith('.com/') and num <= 1 and not 'search' in m_part:
        ...
        readWrite_ptry(arg_str = '-'.join(name.lower().split()), write=True)

        initial_url = m_part, params

    elif 'search' in m_part and num > 1:

        name = m_part + readWrite_ptry() + '/' + str(num) + '/'
...
def cat_site():
    """""" 
    page_download = WebCrawler('https://www.porntry.com/categories/')

    cat_dict = page_download.fetch_list(selector='.letter-block__item a',
                                   attr='href',
                                   title='span',
                                   img='https://img.porntry.com/22613000/22613703/medium@2x/1.jpg',
                                   attr_option = '/www.porntry.com/',
                                   group_dict = True
                                   )
...
def name_site(): 
    """""" 
    page_download = WebCrawler('https://www.porntry.com/models/')
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    mode = re.compile(r"mode=(\w+)?&").search(args).group(1)
    if   mode   == "ptrycat":     site_cat(cat_site)
    elif mode   == "ptryname":    site_name(name_site)
