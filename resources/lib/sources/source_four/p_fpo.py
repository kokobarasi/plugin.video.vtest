...
search_path_fpo = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'
search_file_fpo = os.path.join(xbmc.translatePath('special://home/'), search_path_fpo + "mysearch.txt")

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_fpo.json")
...
path_dicts, mode_fpo_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'fpohome')]
path_dict = load_keys()
...
def readWrite_fpo(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_fpo,'w') as file_handle:
        ...
        with open(search_file_fpo, 'r') as fh:
            
            content = fh.read()
            
            return content

#https://www.fpo.xxx/search/alexis-texas/
def url_link(m_part, num):
    ...
    if num <= 1 and not 'search' in m_part:
        ...
        readWrite_fpo(arg_str = '-'.join(name.lower().split()), write=True)

        initial_url = m_part, params

    elif 'search' in m_part and num > 1:

        name = m_part + readWrite_fpo() + '/' + str(num) + '/'
...
def name_site(): 
    """""" 
    page_download = WebCrawler('https://www.fpo.xxx/models/')

    name_dict = page_download.fetch_names(selector='.item', 
                                title='title',
                                image='src',
                                arg = 3,
                                arg_num = 34,
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if   mode   == "fponames":  site_name(name_site)
