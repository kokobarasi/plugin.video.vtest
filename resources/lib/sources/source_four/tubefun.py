...
search_path_tubefun = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'
dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_tubefun.json")
search_file_tubefun = os.path.join(xbmc.translatePath('special://home/'), search_path_tubefun + "mysearch.txt")
...
path_dicts, mode_tubefun_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'tubefunhome')]
path_dict = load_keys()
...
def readWrite_tubefun(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_tubefun,'w') as file_handle:
        ...
        with open(search_file_tubefun, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if m_part.endswith('.com/'):
        ...
        if not '/search/' in m_part and num <= 1:
            ...
            readWrite_tubefun(arg_str = '-'.join(name.lower().split()), write=True)
            ...
            name = m_part + readWrite_tubefun() + '/page' + str(num) + '.html' 
...
def name_site(): 
    """""" 
    page_download = WebCrawler('https://sextubefun.com/models/')
...
def cat_site(): 
    """""" 
    page_download = WebCrawler('https://sextubefun.com/channels/')
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if   mode   == "tubefunname":  site_name(name_site)
    elif mode   == "tubefuncat":    cat_name(cat_site)
    elif mode   == "tubefuntopname"  :  site_name_filter(name_site_filter)
