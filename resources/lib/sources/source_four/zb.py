...
search_path_zb = 'addons' + '/' + re.search('://(.*)/', base_url_).group(1) + '/' + 'resources' + '/' + 'lib' + '/' + 'sources' + '/' + 'source_three' + '/'

dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_zb.json")
mode_key  = os.path.join(xbmc.translatePath('special://home/'), keys_path + "modelist.json")
search_file_zb = os.path.join(xbmc.translatePath('special://home/'), search_path_zb + "mysearch.txt")
...
path_dicts, mode_zb_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'zbhome')]
path_dict = load_keys()
...
def readWrite_zb(arg_str = None, write=False):
    """Return saved content to memory."""

    if arg_str and write:

        with open(search_file_zb,'w') as file_handle:
        ...
        with open(search_file_zb, 'r') as fh:
...
def url_link(m_part, num):
    ...
    if m_part.endswith('.com/'):
        ...
        if not '/search/' in m_part and num <= 1:
            ...
            readWrite_zb(arg_str = '+'.join(name.lower().split()), write=True)
                
            initial_url = m_part, params

        elif m_part.endswith('/search/') and num > 1:

            name = m_part + readWrite_zb()+ '/' + str(num) + '/' 
...
def name_site(): 
    """""" 
    page_download = WebCrawler('https://zbporn.com/performers/')

    name_dict = page_download.fetch_names(
                                    selector = '.th-image-vertical',
                                    image='src', 
                                    title = "title",
                                    arg = 3,
                                    arg_num = 119,
...
def cat_site(): 
    """""" 
    page_download = WebCrawler('https://zbporn.com/categories/')
...
def channel_site(): 
    """""" 
    page_download = WebCrawler('https://zbporn.com/channels/')

    cat_dict = page_download.fetch_names(
                                    selector = '.thumbs-categories .th-image',
                                    image='src',
                                    image_alt = 'alt',
                                    arg = 3,
                                    arg_num = 4,
...
if re.compile(r"mode=(\w+)?&").search(args):
    ...
    if   mode   == "zbname"      :  site_name(name_site)
    elif mode   == "zbchannel"   :  channel_name(channel_site)
    elif mode   == "zbcat"       :  cat_name(cat_site)
    elif mode   == "zbtopname"  :  site_name_filter(name_site_filter)
