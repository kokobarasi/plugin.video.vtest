...
dict_keys   = os.path.join(xbmc.translatePath('special://home/'), keys_path + "keys_indian.json")
...
path_dicts, mode_indian_list = mode_keys()[0], mode_keys()[number.assign_num(number.mode_keys, 'indiahome')]
path_dict = load_keys()

def url_link(m_part, num):

    if num <= 1 and m_part.endswith('.org/'):

        initial_url = m_part

    elif num > 1 and m_part.endswith('.org/'):

        initial_url = m_part + 'latest-sex' + '/' + str(num) + '/'
...
def fetch_links(m_part, min_search = 1, max_search = 2):
    ...
    movie_src = page_download.playable_links_tup(selector_list = '.videos-list a',
